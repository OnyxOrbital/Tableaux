...
//     console.log("got to sharedTiemtableChange", snapshot.val())
...
//       this.settingstate(myDisplayedData, peopleWhoSharedTheirTTWithMeuid);
...
//    let results = await this.refresh();
//    if (results) {
//      this.setState({ dataReady: true });
//    }
//   }

//   settingstate(myDisplayedData, peopleWhoSharedTheirTTWithMeuid) {
//     this.setState({
//       myDisplayedData: myDisplayedData,
//       peopleWhoSharedTheirTTWithMeuid: peopleWhoSharedTheirTTWithMeuid,
//       // dataReady: true
...
//     console.log('called getshareddata')
...
//     this.setState({ sharedDataReady: true });
...
//     console.log("userlist", userList)
//     if (this.state.dataReady && this.state.sharedDataReady) {
//       if (this.props.firebase.auth.currentUser) {
//         console.log("current userlist", this.state.peopleWhoSharedTheirTTWithMeuid)
...
class SharedTimetable extends React.Component {
  ...
  async getSharedData(peopleWhoSharedTheirTTWithMeuid) {
    ...
    if (peopleWhoSharedTheirTTWithMeuid && peopleWhoSharedTheirTTWithMeuid !== []) {
      // loop through each uid in array to retrieve [username, appointmentsArr]
      peopleWhoSharedTheirTTWithMeuid.forEach(async user => {
        // this.getData(user).then(result => results.push(result));
        results = results.push(await this.getData(user));
      })
    }
    console.log("results", results)
    return results;
     ...
     if (user.uid) {
       ...
       if (value) {
         ...
         if (value.appointments) {
           // console.log(value.appointments.appointmentsArr)
           appointmentsArr.push(Object.values(value.appointments.appointmentsArr));
            return [username, user.uid, appointmentsArr[0], user.sharedAs];
           // console.log("results push", [username, user[0].uid, appointmentsArr[0], user[0].sharedAs])
         } else {
           return [username, user.uid, [], user.sharedAs];
         }
       }
      }
  }
